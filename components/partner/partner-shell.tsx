...
import { HandCoins, LinkIcon, LogOut, ReceiptText } from "lucide-react";

import { logoutPartner } from "@/app/partner/login/actions";
import { Button } from "@/components/ui/button";
import type { PartnerAccess } from "@/lib/supabase/partner";

const items = [
  { href: "/partner", label: "Dashboard", icon: HandCoins },
  { href: "#monthly-earnings", label: "Monthly", icon: ReceiptText },
  { href: "#referral-tools", label: "Referral Link", icon: LinkIcon }
];

export function PartnerShell({ access, children }: { access: PartnerAccess; children: React.ReactNode }) {
  return (
      ...
      <aside className="fixed inset-y-0 left-0 z-30 hidden w-72 border-r bg-[#001A2F] p-4 text-white lg:block">
        <div className="flex items-center gap-3 px-2 py-3">
          ...
          <div>
            ...
            <p className="text-xs font-bold text-white/60">Partner portal</p>
          </div>
        </div>
        ...
        <form action={logoutPartner} className="absolute bottom-4 left-4 right-4">
